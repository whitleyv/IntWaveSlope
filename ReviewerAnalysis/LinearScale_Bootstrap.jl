...
filescalename = apath * "Cheights_" * setname * ".jld2"
dyefile = jldopen(filescalename)

Cut_Cheights = dyefile["Cut_Cheights"]
waves_cut = dyefile["waves_cut"]

(tlength, ylength, numprofslength) = size(Cut_Cheights)

#Let's bootstrap the mean of our data, based on 1000 resamples
nboot = 1000

# basic random sampled bootstrap
bs1 = bootstrap(mean, Cut_Cheights[Cut_Cheights .> 0], BasicSampling(nboot))

#  Var │ Estimate  Bias         StdError
#      │ Float64   Float64      Float64
# ─────┼─────────────────────────────────
#    1 │   92.937  -1.22808e-5  0.375533
# Sampling: BasicSampling
# Samples:  1000
# Data:     Vector{Float64}: { 16937 }

## calculate 95% confidence intervals
cil = 0.95;
bci1 = confint(bs1, BasicConfInt(cil));
# (statistic_value, upper_confidence_bound, lower_confidence_bound)
# ((92.93700183031234, 92.20924012516976, 93.67922890712642),

Cht_havg = zeros(tlength)

for i=1:tlength
    ct = Cut_Cheights[i,:,:]
    Cht_havg[i] = mean(ct[ct .> 0])
end

bs2 = bootstrap(mean, Cht_havg, BasicSampling(nboot))
#==
Bootstrap Sampling
  Estimates:
     Var │ Estimate  Bias       StdError
         │ Float64   Float64    Float64
    ─────┼───────────────────────────────
       1 │  91.7196  -0.156497   3.78383
  Sampling: BasicSampling
  Samples:  1000
  Data:     Vector{Float64}: { 60 }
==#
bs2 = bootstrap(mean, Cht_havg, MaximumEntropySampling(nboot))
bci1 = confint(bs2, BasicConfInt(cil));
...
#==
  Estimates:
     Var │ Estimate  Bias        StdError
         │ Float64   Float64     Float64
    ─────┼────────────────────────────────
       1 │  91.7196  0.00418426   3.84068
  Sampling: MaximumEntropySampling{Float64}
  Samples:  1000
  Data:     Vector{Float64}: { 60 }

# 91.7195955378775, 84.17917017452024, 99.43105631177724
==#
# regular mean is 92.93700183031234

# what if we did maximum entropy in y direction?
Cht_ztavg = zeros(ylength)

for i=1:ylength
    ct = Cut_Cheights[:,i,:]
    if sum(ct .> 0)>0
        Cht_ztavg[i] = mean(ct[ct .> 0])
    end
end

# without the zeros:
bs3 = bootstrap(mean, Cht_ztavg[148:end], MaximumEntropySampling(nboot))
bci3 = confint(bs3, BasicConfInt(cil));
#==
Bootstrap Sampling
  Estimates:
     Var │ Estimate  Bias         StdError
         │ Float64   Float64      Float64
    ─────┼─────────────────────────────────
       1 │   90.683  -0.00226258   1.24843
  Sampling: MaximumEntropySampling{Float64}
  Samples:  1000
  Data:     Vector{Float64}: { 444 }

  ((90.6829909088086, 88.23298667973786, 93.05808211601513),)
==#
