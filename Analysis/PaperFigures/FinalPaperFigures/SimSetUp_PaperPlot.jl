...
f = Figure(resolution = (1500, 1000),fontsize=26) #left, right, bottom, top
    ...
    ax2 = Axis3(gb[1, 1] , azimuth = π/8, # rotation of plot
                ...
                xlabeloffset = 5,
                ylabeloffset = 20, zlabeloffset = 100,
                xlabel="x [m]", ylabel = "y [m]", zlabel = "z [m]",
                yticks = ([800,1600], ["800", "1600"]),
                zticks = ([-250, 0], ["-250","0"]),)
    ax2.title = "Hyperbolic Tangent Initialization"
    limits!((0,152), (0,2000), (-500,0))
    #Box(gb[1, 1], color = (:blue, 0.2), strokewidth = 0)
    ...
    cb = Colorbar(gb[1, 3], limits = (-4,0), colormap = (:thermal), ticks = (-4:1:-1, ["10⁻⁴", "10⁻³", "10⁻²", "10⁻¹"] ),
        label = "Initial Tracer Concentration",lowclip = :white, size = 25)
        #Box(gb[1, 3], color = (:green, 0.2), strokewidth = 0)
    ...
    #ax1.alignmode = Mixed(top=-20)
    #ax2.alignmode = Mixed(right=-100, bottom = 10)
    #ax3.alignmode = Mixed(right=-30, bottom = 10)
    #rowgap!(f.layout, 10)
    ...
    contourf!(ax2, 0:2:154, zz, c2_flat; levels = (-4:0.05:0), colormap = :thermal,
        transformation=(:xz, slope_end+2))
...
save(apath * "SimSetUp.png", f, px_per_unit = 2)
