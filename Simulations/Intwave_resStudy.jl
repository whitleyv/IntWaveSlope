...
model = NonhydrostaticModel(grid = immersed_grid, 
            ...
            boundary_conditions = boundary_conditions,; kwargs_model...)
...
ε = Field(IsotropicPseudoViscousDissipationRate(model))
dBdz = Field(∂z(model.tracers.b))

outputs = merge(outputs, (; ε=ε, N2=dBdz, νₑ=νₑ,))# κₑ=κₑ,))
