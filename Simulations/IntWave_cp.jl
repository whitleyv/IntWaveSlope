...
stop_time_spin = pm.Tσ*4
stop_time_fin = pm.Tfˢ*args["fruntime"]
...
@info "Setting up the spinup model..."
...
spinup_model = NonhydrostaticModel(; tracers = :b, kwargs_model...)
...
set!(spinup_model, u=0, v=0, w=0, b=b₀)
 
###########-------- SIMULATION SET UP ---------------#############
@info "Simultion set up..."

name_prefix = "IntWave_spinup_" * setname
@show data_path = path_name * name_prefix

Δt = 0.8 * underlying_grid.Δyᵃᶜᵃ
spinup_simulation = Simulation(spinup_model, Δt = Δt, stop_time = stop_time_spin)

wizard = TimeStepWizard(cfl=0.5, max_change=1.1, max_Δt=10.0, min_Δt=0.0001) # dec cfl 0.9 -> .5
spinup_simulation.callbacks[:wizard] = Callback(wizard, IterationInterval(5)) # dec. int 500 -> 5 (max)
...
spinup_simulation.callbacks[:progress] = Callback(progress_message, TimeInterval(1200.0))

###########-------- DIAGNOSTICS --------------#############

@info "Adding Diagnostics..."
include("diagnostics.jl")
outputs=get_outputs_tuple(spinup_model, Fulld=args["dflag"])

# don't want to overwrite existing if checkpointed
spinup_simulation.output_writers[:fields] = JLD2OutputWriter(spinup_model, outputs;
                                                          schedule = TimeInterval(600.0),
                                                          filename = data_path,
                                                          overwrite_existing = true)

##########-------- Check Pointer -----------##########
spinup_simulation.output_writers[:checkpointer] = checkpointer = Checkpointer(spinup_model, 
                          schedule=TimeInterval(stop_time_spin), prefix = "checkpoint" * setname)

###########-------- RUN! --------------#############

@info "Run Spinup...."

run!(spinup_simulation)

using Oceananigans.OutputWriters: write_output!
write_output!(checkpointer, spinup_model)

@info "Set up post-spin up model"

model = NonhydrostaticModel(; tracers = (:b, :Cg, :Cs,), kwargs_model...)

run(`nvidia-smi`)

checkpoint_file_path = Oceananigans.OutputWriters.checkpoint_path(true, spinup_simulation.output_writers)
...
set!(model, Cg=c₀1, Cs=c₀2,)

@info "Simultion set up..."

name_prefix = "IntWave_postspin_" * setname
@show data_path2 = path_name * name_prefix

Δt = 0.8 * underlying_grid.Δyᵃᶜᵃ
simulation = Simulation(model, Δt = Δt, stop_time = stop_time_fin)
